  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (request, response, next) => {
        // Handle preflight CORS early so browsers can send Authorization headers.
        if (request.method === 'OPTIONS' && request.url?.startsWith(FUNCTION_ROUTE_PREFIX)) {
        ...
        try {
          ...
          // Ensure local Netlify function responses expose CORS headers and
          // accept the Authorization header so the frontend can authenticate.
          response.setHeader('Access-Control-Allow-Origin', '*');
          response.setHeader('Access-Control-Allow-Headers', 'Authorization,Content-Type');
          ...
          // Ensure CORS headers are present on error paths too.
          response.setHeader('Access-Control-Allow-Origin', '*');
          response.setHeader('Access-Control-Allow-Headers', 'Authorization,Content-Type');
