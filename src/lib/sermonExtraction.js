...
export function isAutomaticStartTime(value) {
  const input = String(value || '').trim().toLowerCase();
  return input === '' || input === 'auto' || input === 'automatic';
}

export function isAutomaticEndTime(value) {
  const input = String(value || '').trim().toLowerCase();
  return input === '' || input === 'auto' || input === 'automatic' || input === 'full';
}
...
export function validateTimecodeRange(startTime, endTime, { allowAutomaticStart = false, allowAutomaticEnd = false } = {}) {
  const automaticStart = allowAutomaticStart && isAutomaticStartTime(startTime);
  const automaticEnd = allowAutomaticEnd && isAutomaticEndTime(endTime);
  const startSeconds = parseTimecodeToSeconds(startTime);
  const endSeconds = parseTimecodeToSeconds(endTime);

  if (!automaticStart && startSeconds === null) {
    return { valid: false, error: 'Start time must use MM:SS or HH:MM:SS.' };
  }

  if (!automaticEnd && endSeconds === null) {
    return { valid: false, error: 'End time must use MM:SS or HH:MM:SS.' };
  }

  if (!automaticStart && !automaticEnd && endSeconds <= startSeconds) {
  ...
  return {
    ...
    automaticStart,
    automaticEnd,
    startSeconds: automaticStart ? null : startSeconds,
    endSeconds: automaticEnd ? null : endSeconds,
    normalizedStartTime: automaticStart ? 'auto' : secondsToTimecode(startSeconds),
    normalizedEndTime: automaticEnd ? 'auto' : secondsToTimecode(endSeconds),
    durationSeconds: automaticStart || automaticEnd ? null : endSeconds - startSeconds,
  };
}
