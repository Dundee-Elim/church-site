...
function Field({ label, value, onChange, type = 'text', placeholder = '', className = '', hint = '' }) {
  return (
...
export default function AdminSermonsPage() {
  ...
  const [dispatchingVideoId, setDispatchingVideoId] = useState('');
  ...
  const audioEpisodeByVideoId = useMemo(() => {
    const pairs = audioEpisodes
      .filter((episode) => episode.video_id || episode.youtube_url)
      .map((episode) => {
        const videoId = episode.video_id || extractYoutubeVideoId(episode.youtube_url);
  ...
  useEffect(() => {
    ...
    if (!['episodes', 'videos'].includes(activeTab) && activeExtractionJobs.length === 0) {
      return undefined;
    }
  ...
  async function handleExtractAudio() {
    ...
    try {
      ...
      admin.setNotice('Audio extraction queued. The worker will pull the YouTube title/date, attach the MP3, and publish the episode automatically.');
    } catch (error) {
  ...
  async function handleExtractVideoAudio(video) {
    if (!video?.id || !video?.watchUrl) {
      admin.setError('This YouTube video is missing the details needed for extraction.');
      return;
    }

    const episodeId = audioEpisodeByVideoId[video.id]?.id || `audio-${video.id}`;
    setDispatchingVideoId(video.id);
    setDispatchingEpisodeId(episodeId);
    ...
    try {
      ...
      admin.setNotice('Audio extraction queued from the YouTube feed. The worker will detect the start, use the full video length, and publish the audio automatically.');
    } catch (error) {
      admin.setError(error.message || 'Unable to queue sermon extraction from this YouTube video.');
    } finally {
      setDispatchingVideoId('');
      setDispatchingEpisodeId('');
    }
  }
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            description="Admins can now queue sermon extraction from a YouTube URL, and the background worker trims, converts, uploads, and attaches the audio automatically. The start time can be typed manually, or left blank/auto so the worker detects the first sustained audio and starts 10 seconds earlier."
          >
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-xl border border-border bg-muted/20 p-4 text-sm text-muted-foreground">
                ...
                <ul className="mt-3 space-y-2">
                  ...
                  <li>Blank or auto start times trigger automatic start detection, then step back 10 seconds.</li>
                  <li>Live YouTube URLs wait until the archive is available, then resume automatically.</li>
                  <li>The finished MP3 is uploaded to Supabase Storage and the episode is published automatically.</li>
                </ul>
              </div>
              <div className="rounded-xl border border-border bg-muted/20 p-4 text-sm text-muted-foreground">
            ...
            >
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field
                  ...
                  placeholder="auto"
                  hint="Leave blank or type auto to detect the first sustained audio/voice and start 10 seconds before it. Use a time like 00:32:15 when you know the exact start."
                />
                <Field
                  ...
                  placeholder="auto"
                  hint="Leave blank or type auto to use the full YouTube video length. Use a time like 01:15:00 when you want to stop earlier."
                />
              </div>
              ...
              <div className="rounded-xl border border-border bg-muted/20 p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">
                      Queue the YouTube worker with the video details above, or use the Video Feed tab so the URL comes from YouTube automatically. Title, date, and full video length can come straight from YouTube. The start can be exact, or blank/auto for automatic detection with a 10 second lead-in.
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2">
        ...
        <TabsContent value="videos" className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {videos.map((video) => {
              const existingAudioEpisode = audioEpisodeByVideoId[video.id];
              const existingJob = existingAudioEpisode ? jobMap[existingAudioEpisode.id] : null;
              const videoDispatching = dispatchingVideoId === video.id || isExtractionActiveStatus(existingJob?.status);
                ...
                <article key={video.id} className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
                  ...
                  <div className="space-y-3 p-5">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {existingAudioEpisode?.audio_url ? <StatusBadge status="published" /> : null}
                      {existingJob ? <ExtractionJobBadge job={existingJob} /> : null}
                    </div>
                    <Button
                      type="button"
                      variant={existingAudioEpisode?.audio_url ? 'outline' : 'default'}
                      onClick={() => handleExtractVideoAudio(video)}
                      disabled={videoDispatching}
                    ...
                    >
                      ...
                      {existingAudioEpisode?.audio_url ? 'Rebuild Audio From YouTube' : 'Publish Audio From YouTube'}
                    </Button>
                  </div>
                </article>
              );
