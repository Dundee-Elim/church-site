        ...
        <div className="mx-auto max-w-7xl">
          ...
          <AnimatePresence>
            {showFilters && activeTab === 'audio' ? (
              <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.18, ease: 'easeOut' }} className="glass-panel relative mx-auto mt-3 p-5">
                {specularLine}
              ...
              {filteredAudio.length === 0 ? (
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {filteredAudio.map((sermon, index) => (
                    <motion.div key={sermon.id || `${sermon.title}-${index}`} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.26, delay: Math.min(index * 0.02, 0.1), ease: 'easeOut' }} className="glass-panel relative flex flex-col gap-4 p-6">
                      {specularLine}
